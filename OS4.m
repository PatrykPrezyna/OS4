            ...
            for bc=1:length(Battery_charger)
                for a=1:length(Autonomous_system)
                    for m=1:length(Motor)
                        ...
                        Design(iteration).Total_weight = Battery(b).weight+Chassis(c).weight+Battery_charger(bc).weight+Autonomous_system(a).weight+(Chassis_pax*PASSENGERS_WEIGHT*0.75)+Motor(m).weight;
                        ...
                        Design(iteration).Peak_Passenger_Throughput  = (Chassis_pax*Number_of_vehicles(d)*Design(iteration).Availability)/Design(iteration).loop_time; 
                        ...
                        Design(iteration).Wait_Time=((805/24)/Design(iteration).Peak_Passenger_Throughput)/2*60;
                        ...
                        % Expected output [0.7237 0.0177 0.0711 0.0032 0.1986]
                        if Design(iteration).Number_of_vehicles == 10 && ...
                            ...
                            if [round(Design(iteration).SAU_availability, 4) round(Design(iteration).SAU_Peak_Passenger_Throughput, 4) ...
                                    ...
                                    round(Design(iteration).MAU, 4)]  == [0.7237 0.0177 0.0711 0.0032 0.1986]
